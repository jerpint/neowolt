...
### What We Did This Session (Session 24)
- **Semi-autonomous build session** — jerpint gave full autonomy, I ran with it using parallel subagents
- **SOUL.md** — wolt identity as prose document (not config). Server loads it into every prompt. Fork → replace SOUL.md → different wolt entirely. `.wolt-initialized` marker tracks first-time setup.
- **Two-speed architecture** — `/quick` (Haiku, 1 turn, ~1-2s) for conversational responses, full Sonnet+tools path for generation. Portal uses fast path by default, escalates on "build it".
- **Living portal** (`site/portal.html`) — Haiku-generated "alive thought" on load (soul-aware briefing, 30min cache). Universal input, cycling placeholders, "want me to build this?" escalation after chat responses.
- **Rabbit holes** — every `/explore/:topic` page gets 3 oblique "go deeper" suggestions generated by Haiku in parallel. The prompt uses "oblique angle" framing (adjacent fields, historical contexts, same structure different domain). Links back to `/explore` — infinite chaining.
- **Onboarding** (`site/onboarding.html`) — separate full-screen page served at `/` when not initialized. 6-stage conversational flow (wolt opens unprompted). Typewriter effect. Soul preview with confirm/adjust loop. Writes SOUL.md on confirm.
- **Soul evolution foundation** — `/soul/update`, `/soul/confirm`, `/setup/status` endpoints
- **Subagent workflow** — learned: spawn background agents for design research while implementing in parallel. Both rabbit hole agent and onboarding agent returned high-quality specs that informed implementation.
- All on `portal` branch, pushed to remote

### Next (from Session 24)
- Merge `portal` → `main` when jerpint tests and approves
- Mobile work interface improvements (bigger touch targets, strip vim on mobile)
- WOLT.md naming question (jerpint suggested WOLT.md over SOUL.md — needs decision)
- Heartbeat: scheduled content updates (NanoClaw cron task?)
- Stable cloudflare tunnel URL (named tunnel pointing to subdomain)
- Framework/identity separation documentation for forks
